...
describe('Stats consumer test', () => {
    ...
    it('Handing message with phase: intermediate, first intermediate message', async () => {
        ...
        await statsConsumer.handleMessage('test_id', 1, {stats_time: statsTime, phase_status: 'intermediate', data: JSON.stringify({ report: {reportId: 1} })});
        ...
        databaseConnectorInsertStatsStub.args[0][0].should.eql('test_id');
        databaseConnectorInsertStatsStub.args[0][1].should.eql(1);
        databaseConnectorInsertStatsStub.args[0][2].should.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
        databaseConnectorInsertStatsStub.args[0][3].should.eql(new Date(Number(statsTime)));
        databaseConnectorInsertStatsStub.args[0][4].should.eql(0);
        databaseConnectorInsertStatsStub.args[0][5].should.eql('intermediate');
        databaseConnectorInsertStatsStub.args[0][6].should.eql(JSON.stringify({report: {reportId: 1 }}));
    ...
    it('Handing message with phase: done', async () => {
        ...
        await statsConsumer.handleMessage('test_id', 1, {stats_time: statsTime, phase_status: 'done', data: JSON.stringify({ report: {reportId: 1} })});
        ...
        databaseConnectorInsertStatsStub.args[0][3].should.eql(new Date(Number(statsTime)));
        databaseConnectorInsertStatsStub.args[0][4].should.eql(0);
        databaseConnectorInsertStatsStub.args[0][5].should.eql('aggregate');
        databaseConnectorInsertStatsStub.args[0][6].should.eql(JSON.stringify({report: {reportId: 1 }}));
    ...
    it('Handing message with phase: aborted', async () => {
        ...
        await statsConsumer.handleMessage('test_id', 1, {stats_time: statsTime, phase_status: 'aborted', data: JSON.stringify({revisionId: 'revision_id', webhooks: ['http://www.zooz.com'], testId: 'test_id', runId: 'run_id', environment: 'test'})});
