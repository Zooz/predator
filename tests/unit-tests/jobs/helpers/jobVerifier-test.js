...
describe('Jobs verifier tests', function () {
    let req, res, resJsonStub, resStatusStub, testsManagerStub, configHandlerStub;
    ...
    describe('verifyTestExists tests', () => {
        it('Should pass test id validation', async () => {
            ...
            await jobVerifier.verifyTestExists(req, res).should.be.fulfilledWith(undefined);
            sandbox.assert.calledOnce(testsManagerStub);
        });

        it('Should fail on test id validation when test not found', async () => {
            req = { body: { test_id: 'id' } };
            testsManagerStub.rejects({ statusCode: 404 });
            await jobVerifier.verifyTestExists(req, res).should.be.rejectedWith(Error, { statusCode: 400, message: 'test with id: id does not exist' });
        });
        ...
        it('Should fail on test id validation when error from performance framework api', async () => {
            ...
            await jobVerifier.verifyTestExists(req, res).should.be.rejectedWith(Error, { statusCode: 500, message: 'failure' });
        });
    });
    ...
    describe('verifyJobBody tests', () => {
        it('Run immediately is true and cron expression does not exist, should pass', async () => {
            ...
            await jobVerifier.verifyJobBody(req, res).should.be.fulfilledWith(undefined);
        });
        ...
        it('Run immediately is false and cron expression does not exist, should fail', async () => {
            ...
            await jobVerifier.verifyJobBody(req, res).should.be.rejectedWith(Error, { statusCode: 400, message: 'Please provide run_immediately or cron_expression in order to schedule a job' });
        });
        ...
        it('Run immediately exists with enabled false, should fail', async () => {
            ...
            await jobVerifier.verifyJobBody(req, res).should.be.rejectedWith(Error, { statusCode: 400, message: 'It is impossible to disable job without cron_expression' });
        });
        ...
        it('Run immediately is false and cron expression exists, cron expression w/ unsupported character, should fail', async () => {
            ...
            await jobVerifier.verifyJobBody(req, res).should.be.rejectedWith(/Unsupported cron_expression./);
        });
        ...
        it('Run immediately with AWS FARGATE without tag in job body, should fail', async () => {
            configHandlerStub.withArgs(consts.CONFIG.JOB_PLATFORM).resolves('AWS_FARGATE');
            req = { body: { run_immediately: true } };
            await jobVerifier.verifyJobBody(req, res).should.be.rejectedWith(Error, { statusCode: 400, message: 'tag must be provided when JOB_PLATFORM is AWS_FARGATE' });
        });
        ...
        it('Run immediately with AWS FARGATE with tag in job body but tag not exists in custom runner definition, should fail', async () => {
            ...
            req = { body: { run_immediately: true, tag: 'hello' } };
            await jobVerifier.verifyJobBody(req, res).should.be.rejectedWith(Error, { statusCode: 400, message: 'custom_runner_definition is missing key for tag: hello' });
        });
    });
});
